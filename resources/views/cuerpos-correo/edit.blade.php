<x-app-layout>
                ...
                <div class="p-6 lg:p-8 bg-white dark:bg-gray-800 dark:bg-gradient-to-bl dark:from-gray-700/50 dark:via-transparent border-b border-gray-200 dark:border-gray-700">
                    <div class="flex items-center justify-between">
                        <h1 class="text-2xl font-medium text-gray-900 dark:text-gray-100">
                            Editar Cuerpo de Correo: {{ $cuerpo->nombre }}
                        </h1>
                    ...
                    <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">
                        Modifica el cuerpo de correo existente
                    </p>
                </div>

                <div class="p-6">
                    <form action="{{ route('cuerpos-correo.update', $cuerpo->id_cuerpo) }}" method="POST" class="space-y-6">
                        @csrf
                        @method('PUT')
                            ...
                            <div>
                                ...
                                <select name="tipo" 
                                    ...
                                    @foreach($tipos as $valor => $nombre)
                                        <option value="{{ $valor }}" {{ old('tipo', $cuerpo->tipo) == $valor ? 'selected' : '' }}>
                                            {{ $nombre }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div>
                            <label class="flex items-center">
                                <input type="checkbox" 
                                       name="activo" 
                                       value="1" 
                                       {{ old('activo', $cuerpo->activo) ? 'checked' : '' }}
                            ...
                            <div>
                                ...
                                <div class="bg-gray-100 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-t-lg p-2 mb-2">
                                 ...
                                 <x-tiptap-editor 
                                     name="cuerpo_html" 
                                     id="cuerpo_html"
                                     :value="old('cuerpo_html', $cuerpo->cuerpo_html)"
                                     :height="400"
                        ...
                        <div>
                            <label for="cuerpo_texto" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                                Cuerpo Texto Plano * (Se genera automáticamente)
                            </label>
                            <textarea name="cuerpo_texto" 
                                      ...
                                      readonly>{{ old('cuerpo_texto', $cuerpo->cuerpo_texto) }}</textarea>
                            @error('cuerpo_texto')
                                <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                            @enderror
                        </div>
    ...
    <div id="modal-variables" class="fixed inset-0 bg-gray-600 bg-opacity-50 hidden z-50">
        ...
        const valoresOriginales = {
            ...
            tipo: '{{ $cuerpo->tipo }}',
            activo: {{ $cuerpo->activo ? 'true' : 'false' }},
            cuerpo_html: '{{ addslashes($cuerpo->cuerpo_html) }}',
            cuerpo_texto: '{{ addslashes($cuerpo->cuerpo_texto) }}'
        };
        ...
        btnPlantillaEjemplo.addEventListener('click', function() {
            ...
            if (confirm('¿Estás seguro de que quieres reemplazar el contenido actual con la plantilla de ejemplo?')) {
        ...
        btnLimpiar.addEventListener('click', function() {
            if (confirm('¿Estás seguro de que quieres restaurar los valores originales? Se perderán todos los cambios.')) {
                document.getElementById('nombre').value = valoresOriginales.nombre;
                tipoSelect.value = valoresOriginales.tipo;
                document.querySelector('input[name="activo"]').checked = valoresOriginales.activo;
                if (editor) {
                    editor.setContent(valoresOriginales.cuerpo_html);
                }
                actualizarCampos();
