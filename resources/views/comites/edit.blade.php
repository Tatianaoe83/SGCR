    ...
    <div class="px-4 sm:px-6 lg:px-8 pt-3 pb-8 w-full max-w-9xl mx-auto">
        ...
        <div class="sm:flex sm:justify-between sm:items-center mb-5">
            <div class="mb-4 sm:mb-0">
                <h1 class="text-2xl md:text-3xl text-gray-800 dark:text-gray-100 font-bold">Editar Comité</h1>
            </div>
        ...
        <div class="bg-white dark:bg-slate-800 shadow-lg rounded-sm border border-slate-200 dark:border-slate-700">
            <div class="p-6">
                <form action="{{ route('comites.update', $comite->relacionID) }}" method="POST">
                    @csrf
                    @method('PUT')
                    @include('comites.partials.form', ['submitLabel' => 'Actualizar Comité'])
                </form>
            </div>
        </div>
    </div>
